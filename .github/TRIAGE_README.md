...
Quick helper — set `TRIAGE_POST_TOKEN` using GitHub CLI (PowerShell)
---------------------------------------------------------------
If you're ready to enable real posting from CI, add a minimal PAT as a repository secret.
Use `gh` on your machine (PowerShell) to set the secret securely:

```powershell
# create a PAT on github.com with the minimal scope (public_repo for public repos, or repo for private)
# then run (you'll be prompted to paste the PAT):
gh secret set TRIAGE_POST_TOKEN --repo sorooh/matrix-platform
```

Notes:
- Keep the PAT minimal and rotate it regularly.
- If you prefer not to add secrets, the DRY_RUN flow (documented above) is safe and will print the comment in logs.
...
<!-- CI trigger: bumped to request check-sharp run 2025-11-03T -->
