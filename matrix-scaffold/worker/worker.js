...
let base = path.join(__dirname, '..', 'matrix-scaffold', 'storage')
// host fallback: try ../storage and ../../storage if the docker-style path doesn't exist
if (!fs.existsSync(base)) {
  const alt1 = path.join(__dirname, '..', 'storage')
  const alt2 = path.join(__dirname, '..', '..', 'storage')
  if (fs.existsSync(alt1)) base = alt1
  else if (fs.existsSync(alt2)) base = alt2
}
...
async function processJob(file) {
  ...
  while (attempt <= RETRY_COUNT) {
    ...
    try {
      ...
      // detect backend port (phase 0 writes runtime/port.txt)
      let port = 3000
      try {
        const rt1 = path.join(base, 'runtime', 'port.txt')
        const rt2 = path.join(path.dirname(base), 'runtime', 'port.txt')
        if (fs.existsSync(rt1)) port = parseInt(fs.readFileSync(rt1, 'utf8').trim(), 10) || 3000
        else if (fs.existsSync(rt2)) port = parseInt(fs.readFileSync(rt2, 'utf8').trim(), 10) || 3000
      } catch (e) {}
      const url = `http://localhost:${port}/apps/${app}`
