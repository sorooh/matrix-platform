    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-text-secondary mt-2">Manage your projects</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects?.map((project: any, index: number) => (
          <motion.div
            ...
            className="bg-surface rounded-lg p-6 border border-border hover:border-primary-500 transition-colors"
          >
            <h3 className="text-xl font-semibold mb-2">{project.name}</h3>
            <p className="text-text-secondary text-sm mb-4">{project.description}</p>
            <div className="flex items-center justify-between text-sm">
              <span className="text-text-secondary">{t('projects.status')}:</span>
