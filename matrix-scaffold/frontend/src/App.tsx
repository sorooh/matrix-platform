...
export default function App() {
  ...
  const [suigKpis, setSuigKpis] = useState<any|null>(null)
  const [suigQ, setSuigQ] = useState('summary')
  const [suigResult, setSuigResult] = useState<any|null>(null)
  const [suigRecs, setSuigRecs] = useState<any|null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const demo = Array.isArray(list) ? list.find((p:any) => p.name === 'Demo Project') : null
        if (demo) setProjectId(demo.id)
        else {
          const created = await fetch('/api/projects', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'Demo Project' }) }).then((r) => r.json())
  ...
  useEffect(() => {
    ...
    es.addEventListener('job.log', (e: any) => {
      try {
        const d = JSON.parse(e.data)
        if (d && d.id && d.id === jobId) {
          setLogs((ls) => [...ls, `[${d.stream}] ${d.chunk}`].slice(-400))
        }
      } catch (err) {}
    })
    es.addEventListener('task.completed', (e: any) => {
      if (!projectId) return
      fetch(`/api/tasks?projectId=${projectId}`).then((r)=>r.json()).then(setTasks).catch(()=>{})
    })
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ marginBottom: 8 }}>
              <button disabled={!projectId} onClick={async () => {
                ...
                setJobStatus(null)
                try {
                  const spec = { kind: 'script', image: 'node:18-bullseye-slim', command: ['node','-e','let i=0;console.log("demo start");const t=setInterval(()=>{console.log("tick",i++); if(i>5){console.log("done");clearInterval(t)}},400)'] }
                  const res = await fetch(`/api/projects/${projectId}/jobs`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ spec }) })
            ...
            <pre style={{ maxHeight: 220, overflow: 'auto', background: '#0b1021', color: '#d6e9ff', padding: 8 }}>{logs.join('')}</pre>
          </div>
          <div style={{ marginTop: 12 }}>
            <h3>Intelligence (SUIG)</h3>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
              <button onClick={async ()=>{ try { const r = await fetch('/api/suig/kpis').then(r=>r.json()); setSuigKpis(r) } catch{} }}>KPIs</button>
              <input value={suigQ} onChange={e=>setSuigQ(e.target.value)} style={{ width: 220 }} />
              <button onClick={async ()=>{ try { const r = await fetch(`/api/suig/query?q=${encodeURIComponent(suigQ)}&scope=all`).then(r=>r.json()); setSuigResult(r) } catch{} }}>Query</button>
              <button disabled={!projectId} onClick={async ()=>{ if(!projectId) return; try { const r = await fetch(`/api/suig/recs?projectId=${projectId}`).then(r=>r.json()); setSuigRecs(r) } catch{} }}>Recs</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <pre style={{ maxHeight: 160, overflow: 'auto', background: '#0b1021', color: '#d6e9ff', padding: 8 }}>{suigKpis ? JSON.stringify(suigKpis, null, 2) : ''}</pre>
              <pre style={{ maxHeight: 160, overflow: 'auto', background: '#0b1021', color: '#d6e9ff', padding: 8 }}>{suigResult ? JSON.stringify(suigResult, null, 2) : ''}</pre>
            </div>
            <div style={{ marginTop: 8 }}>
              <pre style={{ maxHeight: 120, overflow: 'auto', background: '#0b1021', color: '#d6e9ff', padding: 8 }}>{suigRecs ? JSON.stringify(suigRecs, null, 2) : ''}</pre>
            </div>
          </div>
          <div style={{ marginTop: 12, display: 'flex', gap: 16 }}>
            <div style={{ width: '50%' }}>
              ...
              <div style={{ marginBottom: 6 }}>
                <button disabled={!projectId} onClick={async ()=>{ if(!projectId) return; const t = await fetch(`/api/tasks?projectId=${projectId}`).then(r=>r.json()); setTasks(t) }}>Refresh</button>
              </div>
              <ul>
                {tasks.map((t:any)=> (
            ...
            <div style={{ width: '50%' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <input placeholder="query" value={memoryQ} onChange={(e)=>setMemoryQ(e.target.value)} style={{ flex: 1 }} />
                <button disabled={!projectId || !memoryQ} onClick={async ()=>{ if(!projectId) return; const res = await fetch(`/api/memory/${projectId}/search?q=${encodeURIComponent(memoryQ)}`).then(r=>r.json()); setMemoryResults(res) }}>Search</button>
              </div>
              <ul>
                {memoryResults.map((r:any, i:number)=> (
                  <li key={i} style={{ fontSize: 12, marginTop: 6 }}>
                    <div><strong>score:</strong> {r.score?.toFixed?.(3)}</div>
                    <div style={{ whiteSpace: 'pre-wrap' }}>{r.record?.text?.slice(0,240)}</div>
                  </li>
                ))}
              </ul>
            </div>
